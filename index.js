...
const ADMIN_ID   = process.env.ADMIN_ID
  ...
  : process.env.ADMIN_USER_ID ? Number(process.env.ADMIN_USER_ID) : 0;
...
async function sendPairingCode(sock, state, chatId, phoneNumber) {
  if (!phoneNumber || state.sock !== sock || state.status === 'ready' || state.pairingRequested) return;
  ...
  try {
    ...
    if (!pretty) throw new Error('WhatsApp returned an empty pairing code');
    ...
    await bot.sendMessage(
      chatId,
      `⚠️ فشل توليد رمز الربط: ${e.message}`,
      { reply_markup: backKeyboard() }
    ).catch((sendError) => console.error('❌ Pairing error message failed:', sendError));
  }
}
...
async function startSession(userId, chatId, method, phoneNumber) {
  ...
  try {
    ...
    // Pairing-code mode must not depend on a QR update being emitted.
    if (method === 'phone' && phoneNumber) {
      setTimeout(() => {
        sendPairingCode(sock, state, chatId, phoneNumber).catch((e) => {
          console.error('❌ Pairing task failed:', e);
        });
      }, 2500);
    }
...
bot.on('callback_query', async q => {
  console.log('🔘 Callback received:', q?.data || '(empty)');
  try {
    const chatId = q?.message?.chat?.id;
    const userId = q?.from?.id;
    if (!chatId || !userId) {
      console.error('❌ Invalid callback query:', q);
      return;
    }
    ...
    // Acknowledge immediately so Telegram stops showing the button as pending.
    await withTimeout(bot.answerCallbackQuery(q.id), 10000, 'answerCallbackQuery')
      .catch((e) => console.error('⚠️ Could not acknowledge callback:', e.message));
    ...
    if (data.startsWith('admin_kick_')) {
      ...
      await adminKickUser(chatId, data.replace('admin_kick_', ''));
      return;
    }
    if (data.startsWith('admin_sharekey_')) {
      if (!isAdmin(userId)) return;
      const ownerId = Number(data.replace('admin_sharekey_', ''));
    ...
    if (data.startsWith('sharekey_del_')) {
      if (!isAdmin(userId)) return;
      const code = data.replace('sharekey_del_', '');
      deleteShareKey(code);
    ...
    if (data.startsWith('admin_dm_')) {
      if (!isAdmin(userId)) return;
      const tid = data.replace('admin_dm_', '');
    ...
    if (data.startsWith('code_del_')) {
      if (!isAdmin(userId)) return;
      const code = data.replace('code_del_', '');
      deleteCode(code);
    ...
    if (data.startsWith('code_revoke_')) {
      if (!isAdmin(userId)) return;
      const tid = Number(data.replace('code_revoke_', ''));
      revokeUser(tid);
    ...
    if (data.startsWith('admin_ban_')) {
      if (!isAdmin(userId)) return;
      const tid = Number(data.replace('admin_ban_', ''));
      banUser(tid);
      const ts = userStates.get(tid);
      if (ts?.chatId) {
    ...
    if (data.startsWith('admin_unban_')) {
      if (!isAdmin(userId)) return;
      const tid = Number(data.replace('admin_unban_', ''));
      unbanUser(tid);
      const ts = userStates.get(tid);
      if (ts?.chatId) {
    ...
    if (data.startsWith('admin_info_')) {
      if (!isAdmin(userId)) return;
      const tid = Number(data.replace('admin_info_', ''));
      const ts  = userStates.get(tid);
    ...
    if (data.startsWith('admin_use_')) {
      if (!isAdmin(userId)) return;
      const tid = Number(data.replace('admin_use_', ''));
      const ts  = userStates.get(tid);
    ...
    if (data.startsWith('subpfx:')) {
      const prefix  = data.slice(7);
    ...
    try {
      await bot.answerCallbackQuery(q.id, {
        text: 'حدث خطأ داخل البوت',
        show_alert: true
      });
    } catch (answerError) {
      console.error('❌ Callback error response failed:', answerError);
    }
  }
});
